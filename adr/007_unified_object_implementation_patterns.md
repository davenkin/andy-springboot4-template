...
- Command services should not contain business logic but only orchestrate the work of domain model, repositories and
  other components, the business logic should be in AggregateRoots or domain services
- Command Service returns the AggregateRoot's ID for creating objects, and return `void` for updating or deleting
  objects
...
- Use of domain services should be minimized, as domain logic should best be residing in AggregateRoots. Domain
...
- Domain events should hold enough context data about what has happened, but not the whole AggregateRoot or
  non-relevant data
- Domain events are only raised from AggregateRoots by using `AggregateRoot.raiseEvent()` method
